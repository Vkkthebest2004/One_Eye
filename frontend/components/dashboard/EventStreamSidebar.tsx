...
import { ShieldAlert, AlertTriangle, CheckCircle2, Clock, Compass, ArrowUpRight, Flame, HardHat, Shield } from 'lucide-react';
...
export const EventStreamSidebar: React.FC<EventStreamSidebarProps> = ({
  ...
  const getHazardIcon = (hazard: string) => {
    ...
    if (hazard.includes('PROXIMITY')) return <Compass className="w-3.5 h-3.5 text-cyan-400" />;
  ...
  return (
    <div className="flex flex-col gap-unit h-full">
      ...
      <div className="level-1-panel rounded-lg flex-1 overflow-hidden flex flex-col shadow-sm">
        {/* Active Critical Alert Hero Card */}
        {criticalAlert ? (
          <div
          ...
          >
            ...
            <h4 className="font-body-sm text-sm font-semibold text-on-surface leading-tight mb-1 flex items-center gap-1.5">
            ...
            <p className="font-label-mono text-xs text-on-surface-variant mb-2">
              {criticalAlert.worker_id > 0 ? `Worker #${criticalAlert.worker_id.toString().padStart(2, '0')}` : 'Plant-Wide Hazard'} in {criticalAlert.camera_id}.
            </p>

            <div className="grid grid-cols-2 gap-1.5 mb-3">
              <div className="bg-surface-container-lowest border border-outline-variant rounded p-1.5 flex flex-col">
              ...
              <div className="bg-surface-container-lowest border border-outline-variant rounded p-1.5 flex flex-col">
                ...
                <span className="font-label-mono-bold text-xs text-severity-critical">
                  {criticalAlert.risk_score}/100 (CRITICAL)
                </span>
              </div>
            </div>

            {/* Dwell and Distance Telemetry */}
            <div className="grid grid-cols-2 gap-1.5 bg-surface-container-lowest border border-outline-variant rounded p-1.5 text-[10px] font-label-mono mb-3">
              <div className="flex items-center gap-1 text-on-surface-variant">
                <Clock className="w-3 h-3 text-severity-warning" />
                <span>Dwell: <strong className="text-on-surface">{criticalAlert.exposure_seconds?.toFixed(1) || '0.0'}s</strong></span>
              </div>
              <div className="flex items-center gap-1 text-on-surface-variant">
                <Compass className="w-3 h-3 text-primary" />
                <span>Dist: <strong className="text-on-surface">{criticalAlert.distance_m ? `${criticalAlert.distance_m}m` : 'N/A'}</strong></span>
              </div>
            </div>
            ...
            <div className="flex gap-2">
              {criticalAlert.status !== 'ACKNOWLEDGED' ? (
                ...
                <button
                  ...
                  className="flex-1 bg-severity-safe hover:bg-severity-safe/90 text-white font-label-mono-bold text-xs py-1.5 rounded transition-colors flex items-center justify-center gap-1 shadow-sm"
                >
        ...
        {/* Secondary Alerts or Clean Empty State */}
          ...
          {otherAlerts.length > 0 ? (
            otherAlerts.map((w) => (
              <div
                ...
                className={`p-3 border-l-4 transition-colors cursor-pointer bg-surface-container-lowest hover:bg-surface-variant/30 ${
                  w.severity === 'HIGH' ? 'border-l-severity-warning' : 'border-l-primary'
                }`}
              >
                <div className="flex items-center justify-between gap-2 mb-1">
                  ...
                  <span className="font-label-mono text-[10px] text-on-surface-variant font-bold">
                    {w.risk_score}/100
                  </span>
                </div>

                <h4 className="font-body-sm text-xs text-on-surface font-semibold leading-tight mb-1 flex items-center gap-1.5">
                ...
                <div className="flex justify-between items-center text-[10px] font-label-mono text-on-surface-variant mt-1.5">
                  <span className="px-1.5 py-0.5 bg-surface-container border border-outline-variant rounded">
                    {w.camera_id}
                  </span>
            ...
            <div className="flex flex-col items-center justify-center h-full p-6 text-center">
              ...
              <p className="font-label-mono-bold text-xs text-on-surface uppercase tracking-wider">
                All Zones Nominal
              </p>
              <p className="font-label-mono text-[11px] text-on-surface-variant mt-1">
                Zero active safety violations or restricted zone breaches in the dispatch queue.
              </p>
            </div>
          ) : null}
        </div>
