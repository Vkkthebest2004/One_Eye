...
import { Sliders, Camera as CameraIcon, Compass, Shield } from 'lucide-react';
...
export const SettingsTab: React.FC<SettingsTabProps> = ({
  ...
  const [activeSubTab, setActiveSubTab] = useState<'zones' | 'homography'>('zones');
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-surface-container-low p-4 rounded-lg border border-outline-variant">
        <div>
          <h1 className="font-headline-md text-xl font-bold text-on-surface mb-1 flex items-center gap-2">
            ...
            Safety Spatial Zones &amp; Calibration
          </h1>
          <p className="font-label-mono text-xs text-on-surface-variant">
            Draw 2D No-Entry polygons on camera feeds, configure debounce timers, and calibrate planar ground homography.
          </p>
        </div>
        ...
        <div className="flex gap-1.5 bg-surface-container p-1 rounded-DEFAULT border border-outline-variant">
          ...
          <button
            onClick={() => setActiveSubTab('homography')}
            className={`px-3 py-1.5 rounded-DEFAULT font-label-mono-bold text-xs transition ${
              activeSubTab === 'homography'
                ? 'bg-primary text-white shadow-sm font-bold'
                : 'text-on-surface-variant hover:text-on-surface'
            }`}
          >
      ...
      {/* SubTab 1: Interactive 2D No-Entry Polygon Drawing */}
      {activeSubTab === 'zones' && (
        <ZonePolygonEditor
          cameras={cameras}
          zones={zones}
          onZonesUpdated={onConfigSaved}
        />
      )}

      {/* SubTab 2: Planar Metric Homography Ground Calibration */}
      {activeSubTab === 'homography' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-gutter">
          ...
          <div className="lg:col-span-8 level-1-panel rounded-lg p-panel-padding flex flex-col gap-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <select
                  ...
                  className="bg-surface px-2.5 py-1 rounded border border-outline-variant font-label-mono text-xs text-on-surface focus:outline-none font-bold"
                >
                  {cameras.map((c) => (
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 py-2 rounded-DEFAULT bg-primary hover:bg-primary/90 text-white font-label-mono-bold text-xs shadow-sm transition font-bold"
            >
          ...
          <div className="lg:col-span-4 flex flex-col gap-4">
            ...
            <div className="level-1-panel rounded-lg p-panel-padding flex flex-col gap-2 font-label-mono text-xs">
              ...
              <p className="text-on-surface-variant leading-relaxed">
                When calibrated, distances are measured in ground-floor meters via <code>pixel_to_world(x,y)</code>. When uncalibrated, 2D normalized polygonal containment is automatically evaluated.
              </p>
            </div>
          </div>
        </div>
